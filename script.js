const main = () => {
  ...
  orbitCamera.position.set(0.0, 1.4, 0.7*2*3);
  ...
  loader.load(
    ...
    // "boy_1103383614154076073.vrm",
    // "girl_2727424349651793603.vrm",
    config['model'],
    gltf => {
...
const model_map = {
  'boy': "boy_1103383614154076073.vrm",
  'girl': "girl_2727424349651793603.vrm",
}

config = null;
const inputs = ['boy', 'girl']
while (!config) {
  let person = prompt("Enter 'boy' or 'girl' below", "girl");
  if (inputs.includes(person)) {
    config = {
      model: model_map[person]  
    }
  }
}
